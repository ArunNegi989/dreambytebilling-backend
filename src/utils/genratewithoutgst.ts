...
function drawHeader(
  ...
  doc
    ...
    .polygon([0, H - 110], [0, H], [175, H])
    .fill();

  /* DARK GOLD INNER TRIANGLE */
  doc
    .fillColor(COLORS.darkGold)
    .polygon([0, H - 65], [0, H], [120, H])
    .fill();

  /* DARK ACCENT STRIP */
  doc
    .fillColor("#333")
    .polygon([0, H - 35], [0, H], [90, H])
    .fill();
...
function drawInvoiceInfo(doc: PDFKit.PDFDocument, invoice: any) {
  ...
  doc.text("INVOICE", tableX, tableY + 8, {
    width: tableWidth,
    align: "center",
  });
...
/* ---------------- MAIN PDF GENERATION (WITHOUT GST) ---------------- */
export function streamBillWithoutGSTPdf(
  res: Response,
  invoice: any,
  filename = "bill-without-gst.pdf"
) {
